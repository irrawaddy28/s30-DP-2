...
def paint_house(costs, N):
    ...
    recursion
    '''
    def recurse(costs, index, color):
        # index = row index, color = column index
        # base
        if index == N:
           return 0
        ...
        # logic
        if color != 0:
            cost_r = costs[index][0] + recurse(costs, index+1, 0)
        else:
            cost_r = float('inf')

        if color != 1:
            cost_b = costs[index][1] + recurse(costs, index+1, 1)
        else:
            cost_b = float('inf')

        if color != 2:
            cost_g = costs[index][2] + recurse(costs, index+1, 2)
        else:
            cost_g = float('inf')
