...
For eg if we have coins [1,25], amount = 5, dp matrix looks like
  0  1  2  3  4  5
0
1
2
...
Then, dp[i][j] = number of ways required to reach the target amount j using coins[0],...,coins[i-1]
Note: In the dp matrix, row i=0 correspond to a dummy coin of denomination 0.
dp[0][0] = no. ways to reach amount 0 using coin denomination 0 = 1
dp[0][1] = no. ways to reach amount 1 using coin denomination 0 = 0
dp[0][2] = no. ways to reach amount 2 using coin denomination 0 = 0
...
Now, fill the first column using 1's.
dp[1][0] = no. ways to reach amount 0 using coin denomination 1 = 1 way (since
we can choose to not pick coin 1, i.e. [] is 1 way)
dp[2][0] = no. ways to reach amount 0 using coin denomination 2 = 1 way (since
we can choose to not pick coin 2, i.e. [] is 1 way)
...
This completes the initialization step
...
def coin_change2_DP(coins, amount):
    ...
    if N == 0:
        return 0
    if amount == 0:
        return 1 # one way is to not pick any coin
